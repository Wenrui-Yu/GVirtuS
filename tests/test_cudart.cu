...
//     const size_t size = 256 * sizeof(int);
//     int* device_ptr = nullptr;

//     cudaError_t err = cudaMalloc((void**)&device_ptr, size);
//     ASSERT_EQ(err, cudaSuccess) << "cudaMalloc failed with error: " << cudaGetErrorString(err);
//     ASSERT_NE(device_ptr, nullptr) << "cudaMalloc returned null pointer";

//     err = cudaFree(device_ptr);
//     ASSERT_EQ(err, cudaSuccess) << "cudaFree failed with error: " << cudaGetErrorString(err);

//     if (err == cudaSuccess) {
//         std::cout << "[PASSED] cudaMalloc and cudaFree executed successfully." << std::endl;
//     } else {
//         std::cerr << "[FAILED] cudaFree encountered error: " << cudaGetErrorString(err) << std::endl;
//     }
// }
